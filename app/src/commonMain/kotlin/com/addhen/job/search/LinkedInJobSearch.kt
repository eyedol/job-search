...
class LinkedInJobSearchWithKSoup(
    ...
    override suspend fun search(config: JobSearchConfig): List<JobPost> = withContext(Dispatchers.IO) {
        ...
        val url = "${linkedInBaseUrl}keywords=$keywordsParam&location=$locationsParam&f_TPR=r86400&f_WT=2,3&sortBy=DD"
        println(url)
        ...
        ksoup.get(url) {
            elements("div.base-card") {
                eachElement("div.base-card") {
                    val title = select("h3.base-search-card__title") { text().takeIf { it.isNotEmpty() } ?: "Title not found" }
                    val company = select("h4.base-search-card__subtitle") { text().takeIf { it.isNotEmpty() } ?: "Company not found" }
                    val location = select("span.job-search-card__location") { text().takeIf { it.isNotEmpty() } ?: "Location not found" }
                    val link = select("a.base-card__full-link") { attr("href").takeIf { it.isNotEmpty() } ?: "Link not found" }
                    val timePosted = select("time.job-search-card__listdate") { attr("datetime").takeIf { it.isNotEmpty() } ?: LocalDate.now().toString() }
                    val moment = select("time.job-search-card__listdate") { text().takeIf { it.isNotEmpty() } ?: "Moment not found" }

                    val jobPost = JobPost(title, company, location, LocalDate.parse(timePosted), moment, link)
