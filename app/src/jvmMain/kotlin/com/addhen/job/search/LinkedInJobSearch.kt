...
class LinkedInJobSearch(
    ...
    fun login(email: String, password: String) {
        driver(webDriver) {
            ...
            elementByClass("button[type='submit']") {
                click()
            }
            wait(10) {
                urlContains("feed") {
...
inline fun SearchContext.elementById(
    id: String,
    init: WebElement.() -> Unit
) = findElement(By.id(id)).init()

inline fun <T> SearchContext.elementByClass(
    className: String,
    init: WebElement.() -> T
): T = findElement(By.className(className)).init()
